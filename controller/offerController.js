...
cron.schedule('0 0 * * *', async () => {
  ...
  try {
    ...
    for (const offer of activeOffers) {
      try {
        await this.applyOfferToProducts(offer);
      } catch (error) {
    ...
    for (const offer of expiredOffers) {
      try {
        await this.removeOfferFromProducts(offer);
...
class OfferController {
  applyOfferToProducts = async (offer) => {
    const now = new Date();
  ...
  removeOfferFromProducts = async (offer) => {
    const now = new Date();
  ...
  loadAdminOfferPage = async (req, res) => {
    try {
  ...
  editOfferPost = async (req, res) => {
    try {
      ...
      await this.removeOfferFromProducts(offer); // Remove old offer
      ...
      await offer.save();
      await this.applyOfferToProducts(offer); // Apply new offer
  ...
  deleteOffer = async (req, res) => {
    try {
      ...
      if (offer) {
        await this.removeOfferFromProducts(offer); // Remove the offer
